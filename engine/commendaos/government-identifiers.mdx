...
| Type | Description | Format |
|------|-------------|--------|
| `TAX_IDENTIFICATION_NUMBER` | BIR-issued Tax Identification Number (TIN) | `[0-9]{3}-[0-9]{3}-[0-9]{3}(-[0-9]{3})?` |
| `SEC_REGISTRATION_NUMBER` | SEC registration or license number issued upon corporate registration | `[A-Z]{1,4}-?[0-9]{4}-?[0-9]{4,8}` or `[0-9]{6,9}` |
| `VAT_REGISTRATION_NUMBER` | VAT registration number (entity TIN with branch code) | `[0-9]{3}-[0-9]{3}-[0-9]{3}(-[0-9]{3})?` |
| `EMPLOYER_WITHHOLDING_NUMBER` | BIR employer withholding identifier | `[0-9]{3}-[0-9]{3}-[0-9]{3}(-[0-9]{3})?` |
