...
Create a Commenda company document from an already-uploaded file URL and receive a reusable `fileId`.

Use this endpoint after uploading file bytes directly to S3 with [Create a document upload URL](/engine/commendaos/partner-api/commenda-os-documents-upload-url-POST). Pass the `upload.hostedUrl` returned by that endpoint as `fileUrl`. The created file is scoped to the company. You may optionally link it to one reusable Commenda OS resource:
