...
<ParamField body="complianceCategories" type="array">
  ...
  Supported `ObligationRulesCategory` values:

  - `DIRECT_TAX` — corporate income tax, withholding tax, estimated tax, and similar direct-tax filings.
  - `INDIRECT_TAX` — VAT, GST, sales tax, and other indirect-tax returns.
  - `TRANSFER_PRICING` — transfer-pricing reports, master files, local files, and CbC reports.
  - `ENTITY_COMPLIANCE` — corporate-secretarial, statutory, and registration filings (annual reports, beneficial-ownership filings, license renewals).
  - `PAYROLL_AND_EMPLOYMENT` — payroll tax returns, social-security filings, and employment-related obligations.
  - `CUSTOMS_AND_TRADE` — customs, duty, and cross-border trade filings.

  Submit unfiltered (omit the field) to receive every category that applies to the supplied profile.
</ParamField>
...
<ResponseField name="compliances" type="array">
  ...
  <Expandable title="Compliance item">
    ...
    <ResponseField name="category" type="string">
      Obligation category. One of: `DIRECT_TAX`, `INDIRECT_TAX`, `TRANSFER_PRICING`, `ENTITY_COMPLIANCE`, `PAYROLL_AND_EMPLOYMENT`, `CUSTOMS_AND_TRADE`.
    </ResponseField>
