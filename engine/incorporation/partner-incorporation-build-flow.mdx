...
6. Submit incorporation-level intake fields, then read validation until `incorporationValidation.isComplete` is `true`. Participant and document mutations also return the latest `incorporationValidation`, so you can react without an extra read.
...
Check `incorporationValidation.isComplete`. When it is `true`, the incorporation is eligible to submit for Commenda review. Reads return `incorporationValidation` alongside `intakeState`, and write endpoints (intake, participants, documents) return the same summary so a separate read is not required.
