...
| Documents | How do I attach files? | Upload or reuse company documents to obtain `fileId`, register each file to the incorporation, then link it as a typed participant document. |
...
| `fileId` | `POST /partner/commenda-os/companies/{companyId}/documents` or existing Commenda file lookup | Register file; link participant document | Company-scoped file id. The incorporation API registers and links it; upload happens in Commenda OS. |
...
Upload each source document through Commenda OS, or reuse an existing company-visible file, before registering it to the incorporation. If you upload a new file, save the returned `file.fileId`.
