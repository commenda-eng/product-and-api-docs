...
| Documents | How do I attach files? | Follow [Upload documents for incorporation](/engine/incorporation/partner-incorporation-document-upload-flow): upload files directly to S3, create customer documents to obtain `fileId`, then upsert each file as a typed participant document. |
...
| `fileId` | `POST /partner/commenda-os/customers/{customerId}/documents` | Upsert participant document | Customer-scoped file id. Upload bytes directly to S3 first, then create the customer document. |
...
Upload each source document directly to S3, then create a customer document. Upserting the participant document scopes the file to the incorporation and assigns it to the participant document slot in one request.
...
| `participants.participant_123.documents.passport_scan` | A participant is missing a passport scan | Create a customer file, then upsert it as `passport_scan` for that participant. |
...
| `404` when upserting document | The participant or file cannot be found | Register the participant and create the customer file first. |
