...
Only `blocks[].fields[]` keys are valid intake keys. Participant resource fields such as `residentialAddress` must be written to the referenced Commenda OS resource, not to nested intake paths such as `requirements.participants.directors[]` or `requirements.directors[]`.
