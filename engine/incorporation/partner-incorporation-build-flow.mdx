...
| Discovery | What can I incorporate and how will progress display? | Read the jurisdiction catalog, choose `country` plus `countryOptions`, and fetch the workflow stages for the selected case type. |
...
| `stages[].id` | Workflow stages or case status | Progress UI and issue-stage correlation | Stage ids come from the active workflow definition, plus the synthetic `intake` stage. Returned under `workflowStages.stages` (workflow-stages endpoint) or `statusWorkflow.stages` and the case `stages` array (status endpoint). |
...
```bash
curl --request GET \
  --url 'https://api.prod.commenda.io/api/v1/partner/incorporation/workflow-stages?country=SG&countryOptions[corporationType]=PRIVATE_LIMITED_COMPANY' \
  --header 'x-api-key: <partner_api_key>'
```
