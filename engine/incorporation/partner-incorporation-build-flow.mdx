...
| Documents | How do I attach files? | Obtain `fileId`, register it to the incorporation, then link it as a typed participant document. |
...
| `fileId` | Existing Commenda file or shared file upload path | Register file; link participant document | The incorporation API does not upload binary content. |
...
The incorporation API starts from an existing Commenda `fileId`; it does not upload binary content. Obtain `fileId` from an existing Commenda file visible to the company, or from the [shared file upload path](/engine/indirect-tax/files/files-POST) enabled for your integration. If your partner integration does not yet have a file upload path, coordinate with Commenda before building this step.
...
| `conditions` | Rules that make a field required only when another field has a value | Re-evaluate the UI when the controlling field changes. |
...
If validation later becomes incomplete, reads return `collecting_information` again. Public correction, rejection, and webhook flows are not part of the MVP surface.
