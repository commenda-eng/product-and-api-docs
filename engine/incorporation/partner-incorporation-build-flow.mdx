...
description: "Implement a partner-owned incorporation workflow from company creation through ready-for-review validation."
---
...
| Documents | How do I attach files? | Upload files directly to S3, create company documents to obtain `fileId`, register each file to the incorporation, then link it as a typed participant document. |
| Intake | What non-participant fields are needed? | Submit partial or complete intake updates. |
| Completion | How do I know it is ready? | Read the incorporation and check `intakeState.validation.isComplete` and `incorporationStatus`. |
...
| `fileId` | `POST /partner/commenda-os/companies/{companyId}/documents` or existing Commenda file lookup | Register file; link participant document | Company-scoped file id. Upload bytes directly to S3 first, then create the company document. |
...
Upload each source document directly to S3, then create a company document, before registering it to the incorporation. If the file already exists and is visible to the company, you can skip the signed URL step and reuse its `fileId`.
...
Upload each file to the returned `upload.signedUrl` with HTTP `PUT`, then create the company documents from the returned `upload.hostedUrl` values:
...
Intake updates are merge-based and may be partial. Use this endpoint for non-participant data such as company name options and registered address selection.
...
Check `intakeState.validation.isComplete`. When it is `true`, the incorporation is eligible for `ready_for_review`.
...
| `providedRegisteredAddress` | A provided registered address is required | Submit a Commenda OS location reference or choose `standardAddress`. |
...
There is no final submit endpoint in the MVP. Completion is derived from the current state.

```text
collecting_information
  -> ready_for_review   when intakeState.validation.isComplete is true
  -> in_progress        when Commenda starts processing
  -> completed          when the incorporation is complete
```

Before Commenda starts processing, if validation later becomes incomplete, reads return `collecting_information` again. Public correction, rejection, and webhook flows are not part of the MVP surface.
