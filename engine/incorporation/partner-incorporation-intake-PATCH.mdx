...
Each request can be partial. Include only the values you want to add or change in `requirements`. Commenda merges them into the current intake state, validates the incorporation, and returns the updated `intakeState` together with the latest `incorporationValidation` summary.

The Partner API does not expose a submission history endpoint. To read progress after an update, call `GET /partner/incorporation/{incorporationId}` and inspect `intakeState` and `incorporationValidation`.
