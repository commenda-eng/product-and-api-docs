...
The intake endpoint only accepts incorporation-level requirement keys listed in `requirements.blocks[].fields[]`. Do not send participant-scoped keys such as `requirements.participants.directors[].residentialAddress`, `requirements.participants.directors[].resource.residentialAddress`, or `requirements.directors[].residentialAddress`; those are rejected because they would not update the participant's reusable Commenda OS person. For a validation path such as `participants.{participantId}.resource.residentialAddress`, update the referenced person with top-level `residentialAddress` instead.
...
| --- | --- | --- |
| `COMMENDA_SERVICE` | Commenda should provide the registered office address service for this incorporation. | `registeredOfficeAddressSource` |
| `PARTNER_PROVIDED_LOCATION` | You already have a Commenda OS location to use as the registered office address. | `registeredOfficeAddressSource` and `registeredOfficeAddressLocation` |

Use `COMMENDA_SERVICE` when Commenda should provide the registered office address service:
...
Use `PARTNER_PROVIDED_LOCATION` when you are providing a reusable Commenda OS location:
...
If you later switch `registeredOfficeAddressSource` back to `COMMENDA_SERVICE`, any previously submitted `registeredOfficeAddressLocation` value may remain in `currentState`, but it is no longer applicable and will not block validation.
...
Do not satisfy participant residential address requirements with Commenda OS `locations`, `locationTypes`, `CONTACT_ADDRESS`, or `resource.residentialAddress`. The Partner API uses exactly one write interface for this requirement: the person's top-level `residentialAddress` field.
