...
Do not use this endpoint to submit person-level fields such as a Singapore director's residential address. Those fields appear under `participantRequirements[].resourceFieldsByParticipantType` in the requirements response and are satisfied on the referenced Commenda OS person. For a residential address, create or update the person with `residentialAddress`.
