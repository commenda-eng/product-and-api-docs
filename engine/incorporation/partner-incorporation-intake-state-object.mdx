...
`currentState` only contains incorporation-level intake values submitted through `requirements.blocks[].fields[]`. Participant roles, participant-level fields, and participant documents are stored on their own resources and are not represented as nested intake state.
