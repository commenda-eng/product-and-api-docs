...
`currentState` only contains incorporation-level intake values submitted through `requirements.blocks[].fields[]`. Participant roles, participant resource fields such as a person's `residentialAddress`, and participant documents are stored on their own resources and are not represented as nested intake state.
...
| `currentState.registeredOfficeAddressLocation` | `requirements.registeredOfficeAddressLocation` | Commenda OS location reference used when the registered office address source is `PARTNER_PROVIDED_LOCATION`. |
