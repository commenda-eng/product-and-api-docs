...
The intake state object represents the latest merged incorporation-level intake data for one incorporation. Use it together with `incorporationValidation` to decide what still needs to be collected before submitting the incorporation for Commenda review.
...
`incorporationValidation` is returned alongside `intakeState` on incorporation reads and writes. Endpoints that mutate intake-affecting data &mdash; updating intake, registering, updating, or removing a participant, and linking a participant document &mdash; return the latest validation summary in the response so partners can render readiness state without an extra round trip.

| Attribute | Type | Description |
| --- | --- | --- |
| `incorporationValidation.isComplete` | boolean | `true` when the current incorporation state satisfies the current requirements and is eligible to submit for review. This does not submit the incorporation by itself. |
| `incorporationValidation.missingRequirements` | array | Required fields, participants, ownership totals, or participant documents that have not been provided yet. |
| `incorporationValidation.invalidRequirements` | array | Submitted values that do not satisfy the current requirements. |
