...
`incorporationValidation.isComplete` means the current data is eligible to submit. It does not submit the incorporation by itself. Reads of the incorporation, intake updates, and participant or document mutations all return the latest `incorporationValidation` summary so you can react without an extra request.
