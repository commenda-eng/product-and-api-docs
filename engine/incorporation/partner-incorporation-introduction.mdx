...
3. Fetch the current requirements DSL with `GET /partner/incorporation/requirements` and the progress stages with `GET /partner/incorporation/workflow-stages` for the selected `country` and `countryOptions`.
