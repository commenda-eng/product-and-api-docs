...
| Field | Description |
| --- | --- |
| `incorporationValidation` | Current validation summary for the incorporation. Returned by reads (Get an incorporation) and by intake, participant, and document write endpoints so partners can render readiness state without an extra round trip. |
| `requirements` | Current incorporation requirements for the selected jurisdiction and options. |
| `intakeState` | Current submitted intake values. |
