...
The response includes the registered `participant` plus the latest `incorporationValidation` summary so you can decide what to collect next without an extra read. Use `incorporationValidation.missingRequirements` and `incorporationValidation.invalidRequirements` to drive UI for participant roles, ownership totals, and required participant documents.
