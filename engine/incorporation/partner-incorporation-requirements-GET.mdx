...
Validation paths in `missingRequirements` and `invalidRequirements` correspond to DSL fields or participant/document requirements. For example, `companyNameOptions` points to an intake field, while `participants.participant_123.documents.passport_scan` points to a missing participant document.
