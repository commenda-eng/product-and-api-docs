...
Status stages use the same field names as the status workflow: `id`, `label`, `description`, `owner`, and `position`. The `stageId` field appears only inside issue scopes, where it references a stage's `id`.
...
Status stages do not return `stageId` or `name`.
