...
| Attribute | Type | Description |
| --- | --- | --- |
| `code` | string | Stable machine-readable validation issue code. Current values include `REQUIREMENT_MISSING`, `REQUIREMENT_INVALID`, `PARTICIPANT_REQUIREMENT_INVALID`, `PARTICIPANT_DOCUMENT_REQUIRED`, `SHAREHOLDER_REQUIREMENT_INVALID`, and `SHAREHOLDER_OWNERSHIP_INVALID`. Treat the list as extensible. |
...
| `participantId` | string | Present when the issue belongs to a participant. Use this field instead of extracting the participant id from `path`. |
| `documentKind` | string | Present when the issue belongs to a participant document. Use this field instead of extracting the document kind from `path`. |
| `displayName` | string | Present when Commenda can provide a useful label for the affected person, business entity, or field. |

Use structured fields before falling back to `path`. For example, a UI that renders document upload tasks can group issues by `participantId` and `documentKind` without parsing `participants.{participantId}.documents.{documentKind}`.
