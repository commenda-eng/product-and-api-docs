...
- **EU countries (e.g., DE, FR):** `address_line_1`, `city`, and `postal_code` are `REQUIRED`, while `state` is `OPTIONAL`. Postal codes are validated against the country's regex pattern (e.g., `^\d{5}$` for DE and FR).
...
<Note>
  Even when a field is `OPTIONAL`, providing it improves tax calculation accuracy. For example, including a state code for EU or GB addresses can help resolve the correct tax jurisdiction.
</Note>
...
<Note>
The `postal_code` field is **required** for the US, EU countries (such as DE and FR), and GB. For CA, postal code is validated against the country's regex pattern when provided. Postal codes are always validated against the country-specific regex when present.
</Note>
