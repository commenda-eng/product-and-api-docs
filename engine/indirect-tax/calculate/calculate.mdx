...
- **Postal code format** — the postal code must match the country's expected format (e.g., `^\d{5}(-\d{4})?$` for US ZIP codes). The `postal_code` field is required for the US, EU countries (such as DE and FR), and GB. For CA, providing a postal code is recommended for accurate tax jurisdiction resolution.
