...
2. `/transactions` endpoints:
   ...
   - Supports both sales (`SALE`) and purchase (`PURCHASE`) transaction types. Purchase transactions are used for input tax credit calculations (EU VAT / UK VAT) and are excluded from sales-focused flows like nexus and filings.
