...
| tax_remitter_type | false | [Tax Remitter Type](#tax-remitter-type) | Indicates which party is responsible for remitting tax on the transaction. |
| is_triangulation | false | boolean | Marks the row as part of an invoice that is the middle leg of an intra-EU triangulation chain (A→B→C across three EU member states, where the seller acts as B). All rows belonging to the same invoice (grouped by `invoice_number` / `external_id`) must carry the same value; rows whose values disagree cause the invoice to be rejected. |
...
| SUB_TRANSACTIONS_TRIANGULATION_INCONSISTENT | Rows belonging to the same invoice have disagreeing `is_triangulation` values | Ensure every row that shares the same `invoice_number` carries the same `is_triangulation` value |
...
Currently the following options are supported: `PURPOSE_RESALE`, `ENTITY_TYPE_NONPROFIT`, `GOVERNMENT`, `MANUFACTURER`, `AGRICULTURAL`, `EXEMPT_PRODUCT`, `OTHER`.

#### Tax Remitter Type

Indicates which party is responsible for remitting the tax on the transaction. This field is relevant for VAT and reverse-charge scenarios.
