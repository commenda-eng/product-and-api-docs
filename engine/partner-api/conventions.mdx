...
| --- | --- | --- | --- |
| Customer | `customerId` | integer | Numeric id for the customer that owns the work. |
...
| Participant | `participantId` or `participant.id` | string | Opaque id for one participant registration in an incorporation. |
...
Commenda-defined enum values use lowerCamelCase.

```json
{
  "incorporationStatus": "awaitingCustomerInput",
  "documentKind": "passportScan",
...
## Backward-compatible rollout checklist

When updating an existing Partner API endpoint to match these conventions:

1. Update the OpenAPI schema first so generated clients show the intended contract.
2. Update docs prose and examples at the same time.
3. Update backend serializers at the Partner API boundary. Do not rename keys inside opaque customer-provided payloads.
4. Add tests that assert public field names, ID types, wrappers, enum values, and error statuses.
5. If partners already depend on the old shape, provide an explicit migration plan before removing the old fields.
