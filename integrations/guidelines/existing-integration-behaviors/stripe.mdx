...
Customer records continue to sync as source records and snapshots through Rootfi, but their addresses are __not__ eagerly geocoded or pushed into the sales-tax address pipeline. The eager customer-address path is skipped for `PAYMENT_CUSTOMERS`, so `rawAddressString` is left null unless another path explicitly sets it. Invoice-level (`PAYMENT_INVOICES`) and credit-note (`PAYMENT_CREDIT_NOTES`) address resolution — including the transaction → customer fallback described above — is unchanged.
