...
When syncing invoices, Commenda also populates the `customer_business_identification_number` field on each transaction using the associated contact's tax number. This ensures the customer's tax identification is available directly on the transaction for reporting and filing purposes.
